...
* ``/inventory/restock`` GET to refill the stock of all items. comes in handy if inventory is empty (as in 'everything is sold out')
* ``/inventory/generate`` GET to generate inventory db entries. in case someone forgot to start the db before the inventory. 
